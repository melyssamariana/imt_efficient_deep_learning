...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Search best score with FP16 + structured pruning + unstructured pruning + retraining "
            "on ResNet20/CIFAR-10."
        )
    )
    parser.add_argument(
        "--checkpoint",
    ...
    parser.add_argument("--q-w", type=int, default=16, help="Bits for weight precision (FP16 -> 16).")
    parser.add_argument("--q-a", type=int, default=16, help="Bits for activation precision (FP16 -> 16).")
    parser.add_argument(
        "--w-ref",
        type=float,
        default=1.0,
    ...
    parser.add_argument(
        "--output-json",
        default="/home/antonio/imt_efficient_deep_learning/antonio/experiments/quantization/fp16_structured_unstructured_search_results.json",
    )
    parser.add_argument(
        "--output-plot",
        default="/home/antonio/imt_efficient_deep_learning/antonio/experiments/quantization/fp16_structured_unstructured_search_plot.png",
    )
...
def quantize_weights_fp16_storage(model: nn.Module):
    with torch.no_grad():
...
def evaluate(model, loader, device, use_native_fp16: bool, max_batches=None):
...
def finetune(
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            # Keep CPU path close to FP16 storage semantics.
            if not use_native_fp16:
                quantize_weights_fp16_storage(model)
...
def print_model_summary(model: nn.Module, device: torch.device, use_native_fp16: bool):
    summary_input = torch.randn(
        1,
        3,
        32,
        32,
        device=device,
        dtype=torch.float16 if use_native_fp16 else torch.float32,
    )
    summary(model, input_data=summary_input)
...
def save_scatter_plot(results: List[SearchResult], output_path: str, acc_threshold: float):
    ...
    sc = ax.scatter(scores, accs, c=colors, cmap="plasma", s=80)
    ...
    ax.set_title("FP16 + Structured + Unstructured Search")
    ax.grid(alpha=0.3)
    ax.legend()
    plt.tight_layout()
...
def main():
    ...
    use_native_fp16 = device.type == "cuda"

    print("use_native_fp16:", use_native_fp16)
    ...
    print("Method: FP16 + structured pruning + unstructured pruning + retrain")
    print(f"Precision path: {'native FP16 (model+inputs)' if use_native_fp16 else 'FP16 storage emulation on CPU'}")
    ...
    # Use default w_ref and f_ref if not provided, to make raw_score equivalent to the combined score.
    args.w_ref = 5.6 * 10**6
    args.f_ref = 2.8 * 10**8
    ...
    for p_s in sorted(set(args.structured_ratios)):
        for p_u_rel in sorted(set(args.unstructured_ratios)):
            ...
            if use_native_fp16:
                print("Converting model to native FP16 precision.")
                model = model.half()
            else:
                print("Using FP16 storage emulation on CPU (model remains in float32, weights quantized to FP16 semantics).")
                quantize_weights_fp16_storage(model)


            print_model_summary(model, device=device, use_native_fp16=use_native_fp16)
    ...
    payload = {
        ...
        "device": str(device),
        "precision_mode": "native_fp16" if use_native_fp16 else "fp16_storage_emulation_cpu",
        "q_w": args.q_w,
        "q_a": args.q_a,
        "w": w,
        "f": f,
